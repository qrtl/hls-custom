...
from odoo.addons.stock_svl_removal_date.hooks import fill_removal_date
...
class TestStockSvlRemovalDate(BaseCommon):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
    ...
    def test_expiration_date_change_realigns_layers(self):
        ...
        Such a recomputation is flushed without going through write(), so the
        realignment cannot rely on removal_date being part of its vals.
        """
    ...
    def test_fill_removal_date_hook(self):
        """The SQL of the install hook fills the same dates as create() does."""
