...
class StockOutgoingShipmentReport(models.TransientModel):
    ...
    delivery_date = fields.Char(string="Delivery Date", compute="_compute_date_fields", store=True)
    expected_date = fields.Char(string="Expected Date", compute="_compute_date_fields", store=True)
    ...
    expiry_date = fields.Char(string="Expiry Date", compute="_compute_date_fields", store=True)
                ...
                if prop[0] == "Float":
                    try:
                        ...
                        try:
                            ...
                            raise ValidationError(msg % _(rec.fields_get(field)[field].get('string')))
