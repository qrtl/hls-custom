...
class TestAccountPaymentImportSbt(SavepointCase):
    ...
    def setUpClass(cls):
        ...
        cls.sale_order = cls.env["sale.order"].create(
            {"partner_id": cls.partner.id, 'carrier_id': cls.env.ref('delivery.free_delivery_carrier').id}
        )
    ...
    def test_01_get_shipping_address(self):
        self.assertEqual(self.partner._get_shipping_address(), "AomoriCityStreetStreet2")
    ...
    def test_02_generate_stock_outgoing_shipment_report(self):
        ...
        self.assertEqual(test_line.carrier_name, self.env.ref('delivery.free_delivery_carrier').name[:20])
